...
/**
 * searchPath - search for executable from PATH
 * @params: parameters struct
 * Return: return fullPath if success or NULL if error
*/
char *searchPath(param *params)
{
	...
	for (pathToken = _strtok(path, ":", &state);
			pathToken; pathToken = _strtok(path, ":", &state))
	{
		temp = fullPath;
